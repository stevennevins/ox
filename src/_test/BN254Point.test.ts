import { BN254, BN254Point } from 'ox'
...
describe('fromBytes', () => {
  test('G1', () => {
    ...
    const publicKeyHex = BN254Point.toBytes(publicKey)
    expect(BN254Point.fromBytes(publicKeyHex, 'G1')).toEqual(publicKey)
  })
  ...
  test('G2', () => {
    ...
    const publicKeyHex = BN254Point.toBytes(publicKey)
    expect(BN254Point.fromBytes(publicKeyHex, 'G2')).toEqual(publicKey)
  })
})
...
describe('fromHex', () => {
  test('G1', () => {
    const publicKey = BN254.getPublicKey({ privateKey })
    const publicKeyHex = BN254Point.toHex(publicKey)
    expect(BN254Point.fromHex(publicKeyHex, 'G1')).toEqual(publicKey)
  })
  ...
  test('G2', () => {
    ...
    const publicKeyHex = BN254Point.toHex(publicKey)
    expect(BN254Point.fromHex(publicKeyHex, 'G2')).toEqual(publicKey)
  })
})
