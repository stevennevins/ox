...
describe('aggregate', () => {
  test('default', () => {
    ...
    const privateKeys = Array.from({ length: 100 }, () =>
      BN254.randomPrivateKey(),
    )

    const signatures = privateKeys.map((privateKey) =>
      BN254.sign({ payload, privateKey }),
    )
    const signature = BN254.aggregate(signatures)

    const publicKeys = privateKeys.map((privateKey) =>
      BN254.getPublicKey({ privateKey }),
    )
    const publicKey = BN254.aggregate(publicKeys)

    const valid = BN254.verify({
      payload,
      publicKey,
      signature,
    })
  ...
  test('size: "long-key:short-sig"', () => {
    ...
    const signatures = privateKeys.map((privateKey) =>
      BN254.sign({ payload, privateKey, size: 'long-key:short-sig' }),
    )
    const signature = BN254.aggregate(signatures)

    const publicKeys = privateKeys.map((privateKey) =>
      BN254.getPublicKey({ privateKey, size: 'long-key:short-sig' }),
    )
    const publicKey = BN254.aggregate(publicKeys)
...
describe('getPublicKey', () => {
  it('default', () => {
    const publicKey = BN254.getPublicKey({ privateKey })
  ...
  it('size: "long-key:short-sig"', () => {
    const publicKey = BN254.getPublicKey({
      privateKey,
...
describe('randomPrivateKey', () => {
  it('default', () => {
    const privateKey = BN254.randomPrivateKey()
    expect(privateKey).toBeDefined()
    expect(privateKey.length).toBe(66)
  })

  it('as: bytes', () => {
    const privateKey = BN254.randomPrivateKey({ as: 'Bytes' })
    expect(privateKey).toBeDefined()
...
describe('sign', () => {
  test('default', () => {
    ...
    const signature = BN254.sign({ payload, privateKey })
...
describe('verify', () => {
  test('default', () => {
    ...
    const signature = BN254.sign({ payload, privateKey })
    const publicKey = BN254.getPublicKey({ privateKey })
    const verified = BN254.verify({ payload, publicKey, signature })
